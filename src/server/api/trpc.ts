...
import { type Session } from "./routers/login";

export async function decodeSession(req: NextApiRequest) {
  console.log(req.headers.authorization, "sdsd");
  if (req.headers.authorization) {
    try {
      ...
      console.log("session________________", session);
      if (session) {
        return {
          id: session.id,
...
export const createTRPCContext = async (opts: CreateNextContextOptions) => {
  const { req } = opts;
  console.log("in");
  const session = (await decodeSession(req)) ?? null;
